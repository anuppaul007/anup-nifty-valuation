...
This does NOT assert which vintage Nifty used. It asks whether the residual
hard-bound breaches are consistent with a quarterly IWF update lag. Public
shareholding remains an upper bound, never an IWF substitute.
"""
...
def main():
    ...
    for row in weights:
        ...
        try:
            ...
            mp=b.fnum(mar.get('public_val'));jp=b.fnum(jun.get('public_val'))
            if mp is None or jp is None:raise ValueError('public_pct_missing')
            done.append({**row,'shareholding_total_shares_june':shares,'implied_iwf':point,'implied_iwf_min':lo,'implied_iwf_max':hi,
                         'public_pct_mar':mp,'public_pct_jun':jp,'public_change_pp_jun_minus_mar':jp-mp,
                         'breach_vs_mar':bool(lo>mp/100.0),'breach_vs_jun':bool(lo>jp/100.0),
    ...
    changed=[{'symbol':x['symbol'],'public_mar':x['public_pct_mar'],'public_jun':x['public_pct_jun'],'change_pp':x['public_change_pp_jun_minus_mar'],'implied_iwf_pct':100*x['implied_iwf'],'breach_mar':x['breach_vs_mar'],'breach_jun':x['breach_vs_jun']} for x in done if x['breach_vs_mar']!=x['breach_vs_jun']]
    out={'schema_version':1,'generated_at':datetime.now(timezone.utc).replace(microsecond=0).isoformat(),'research_only':True,'anchor_date':'2026-08-31','coverage':{'ok':len(done),'required':50,'failed':len(errors)},'failed':errors,'breaches_vs_mar_public':block(bm),'breaches_vs_jun_public':block(bj),'breach_status_changes':changed,'weight_source':wmeta,'interpretation_guardrail':'This is a timing-consistency audit only. A non-breach versus March does not prove March IWF; it only shows compatibility with an earlier public-shareholding upper bound.','constituents':done,'live_model_changed':False,'live_allocation_changed':False,'tolerances_changed':False}
    ...
    print(json.dumps({'coverage':out['coverage'],'breaches_vs_mar_public':out['breaches_vs_mar_public'],'breaches_vs_jun_public':out['breaches_vs_jun_public'],'breach_status_changes':changed},indent=2))
