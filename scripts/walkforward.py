...
def overlay_damp(z,zc):return 1.0 if float(z)<0 else clip(1-abs(float(z))/float(zc),0,1)
def add_month(month,n):return str(pd.Period(month,freq='M')+n)

def model_snapshot(d,now=None):
    n=d.get('nifty') or {};m=d.get('macro') or {};e=d.get('earnings') or {};t=d.get('trend') or {}
