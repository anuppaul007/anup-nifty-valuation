...
out-of-sample record before either the macro-overlay cap or the valuation-to-
allocation curve can be changed.
"""
...
# Must stay in sync with model.js. Any future live change should update tests.
...
def curve(z,k=None):
    k=C['k'] if k is None else float(k)
    f=lambda x:100/(1+math.exp(k*x));lo=f(C['zc']);hi=f(-C['zc'])
...
def model_snapshot(d):
    ...
    curve_candidates={str(k):clip(curve(z,k)+ea+ms*C['macroMax']*damp,0,100) for k in CANDIDATE_CURVE_SLOPES}
    ...
    return {
      ...
      'candidate_equity_targets':macro_candidates,
      'candidate_curve_targets':curve_candidates,
...
def summary(records):
    ...
    return {
      ...
      'parameter_lock':'macroMax=6 pp and allocation-curve k=1.35 remain locked until prospective evidence gate is satisfied',
...
def main():
    ...
    out={'schema_version':1,'model_version':'3.6','updated_at':datetime.now(timezone.utc).isoformat(timespec='seconds'),
         'candidate_macro_caps_pp':CANDIDATE_MACRO_CAPS,'candidate_curve_slopes':CANDIDATE_CURVE_SLOPES,
