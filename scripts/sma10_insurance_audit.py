...
2) the longest exact shared NIFTY 50 TRI / NIFTY 10YR Benchmark G-Sec history
   available to the already-frozen trend challenger.

This script is research-only and never modifies model.js or live parameters.
"""
...
def simulate_window(market,execs,policy,start_period,end_period):
    ...
    last=eligible_end.max()
    m=market.loc[first:last]
    b,bf,c,cf=a.run_variant(m,chosen,policy,0.0,0.001)
    return b,c,{
      ...
      'trend_executions':int(len(chosen)),
      'risk_off_months':int(chosen.risk_off.sum()),
...
def main():
    ...
    # Reconstruct the exact monthly return-month labels used in robust_evaluation.
    retro=json.loads((ROOT/'data'/'retrospective.json').read_text(encoding='utf-8'))
    panel=re.panel_from_retrospective(retro)
    frame,_,_,_=r.strategy_returns(panel,r.LIVE_K,r.LIVE_ZC)
    ...
    fb,_,fc,_=a.run_variant(market,execs,policy,0.0,0.001)
    fullmeta={
      'first_execution':str(execs.index.min().date()),'last_market_date':str(market.index.max().date()),
      'trend_executions':int(len(execs)),'risk_off_months':int(execs.risk_off.sum()),
      'risk_off_pct':100*float(execs.risk_off.mean())
    }
    ...
    out={
      ...
      'longest_exact_shared_history':{**fullmeta,**tradeoff(fb,fc)},
    ...
    print(json.dumps({
      'same63':{
        'cagr_premium_pp':out['same_63_calendar_return_months']['cagr_premium_paid_pp_per_year'],
        'max_dd_avoided_pp':out['same_63_calendar_return_months']['max_daily_drawdown_avoided_pp']},
      'longest':{
        'cagr_premium_pp':out['longest_exact_shared_history']['cagr_premium_paid_pp_per_year'],
        'max_dd_avoided_pp':out['longest_exact_shared_history']['max_daily_drawdown_avoided_pp']}
    },separators=(',',':')))
