...
function fixture(){return{schema_version:4,generated_at:'2026-09-11T11:46:28Z',nifty:{date:'2026-09-10',level:23477.8,pe:19.85,pb:2.84,div_yield:1.21,gsec10:6.88,gsec_meta:{asof:'2026-09-10',status:'live',max_age_days:7}},earnings:{score:-.3201937701240839,asof:'2026-08-31',coverage:1},macro:{score:-.3061910122582852,active_block_weight:.53}};}
test('known snapshot arithmetic and coverage attenuate the macro budget',()=>{
 const r=M.calculate(fixture(),now);
 assert(Math.abs(r.core-83.1)<.06);
 assert(Math.abs(r.ma-(-.3061910122582852*6*r.damp*.53))<1e-12);
 assert(Math.abs(r.L.reduce((s,x)=>s+x.weight,0)-100)<1e-12);
 assert(Math.abs(r.final-(r.core+r.ea+r.ma))<1e-12);
});
test('0 and 100 equity remain attainable and curve is monotone',()=>{
...
test('unknown or stale macro cannot affect allocation',()=>{
 for(const change of [d=>d.macro.score=null,d=>d.macro_stale=true,d=>d.generated_at='2026-09-01',d=>d.schema_version=3]){
  const d=fixture();change(d);assert.equal(M.calculate(d,now).ma,0);
 }
});
test('less coverage reduces the absolute overlay, without changing the score',()=>{
 const d=fixture();const full=M.calculate(d,now);d.macro.active_block_weight=.1;
 assert(Math.abs(M.calculate(d,now).ma)<Math.abs(full.ma));
});
...
test('failed browser reload clears a previously displayed allocation',()=>{
 const nodes=new Map();const node=id=>nodes.get(id)||nodes.set(id,{textContent:'81%',style:{},className:''}).get(id);
