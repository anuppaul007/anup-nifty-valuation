...
function fixture(){
 ...
 d.generated_at='2026-09-11T11:46:28Z';d.model_version='3.11-crash-aware-1';d.nifty={date:'2026-09-10',level:23477.8,pe:19.85,pb:2.84,div_yield:1.21,gsec10:6.88,gsec_meta:{asof:'2026-09-10',status:'live',max_age_days:7}};
...
test('known snapshot arithmetic works when complete gates pass',()=>{
 const r=M.calculate(fixture(),now);
 assert(Math.abs(r.core-81.14682556581988)<1e-10);assert.equal(r.coverage,1);
 assert.equal(r.macroEligible,true);assert.equal(r.trendEligible,true);assert.equal(r.allocationReady,true);
 assert.equal(r.damp,1);assert(Math.abs(r.ma-(-.3061910122582852*6))<1e-12);
 assert.equal(r.ta,0);assert(Math.abs(r.L.reduce((s,x)=>s+x.weight,0)-100)<1e-12);
 assert(Math.abs(r.final-(r.core+r.ea+r.ma+r.ta))<1e-12);
});
...
test('cheap-side macro and earnings authority no longer vanish at valuation extremes',()=>{
 ...
 assert(r.z<0);assert.equal(r.damp,1);assert.equal(r.ma,-6);assert.equal(r.ea,-6);assert(r.final<=88+1e-9);
});
test('macro worsens allocation monotonically across valuations while expensive-side damping remains bounded',()=>{
