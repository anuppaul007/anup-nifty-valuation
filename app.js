...
function render(d){
 ...
 q('#verdict').textContent=r.allocationReady?`Valuation is ${band(r.z)} against the model’s fixed references. With every required macroeconomic block verified, the strategic model target is ${r.final.toFixed(1)}% equity / ${(100-r.final).toFixed(1)}% debt. This remains a research allocation rule; overlay strength will only be changed after walk-forward evidence.`:`No equity/debt target is published because ${r.holdReason||'the complete-data gate is not satisfied'}. Fundamental valuation is still shown, but incomplete macro evidence is never converted into a precise allocation.`;
