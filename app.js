...
function render(d){
 ...
 q('#verdict').textContent=r.allocationReady?`Valuation is ${band(r.z)} against the model’s fixed references. With every required macroeconomic block verified, the strategic model target is ${r.final.toFixed(1)}% equity / ${(100-r.final).toFixed(1)}% debt. This is a research allocation rule; the macro influence cap is not yet return-optimized by walk-forward backtesting.`:`No equity/debt target is published because ${r.holdReason||'the complete-data gate is not satisfied'}. Fundamental valuation is still shown, but incomplete macro evidence is never converted into a precise allocation.`;
 ...
 const names={global_liquidity:'Global financial conditions',india_external_carry:'India external / carry',china_industrial:'China / global industrial cycle'},details=m.block_weight_detail||{},base=C.macroRequiredWeight||.8;
 ...
 const ch=m.china_pmi||{},factors=m.factors||{},packetOk=d.schema_version===4&&fresh(d.generated_at,3);
 ...
 const issues=Object.entries(requiredFactorNames).filter(([k])=>factors[k]?.status!=='live').map(([,v])=>v);if(ch.status!=='live')issues.push('China PMI');
